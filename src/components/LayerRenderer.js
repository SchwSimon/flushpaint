 ...
 */
class LayerRenderer extends Component {
	constructor(props) {
		super(props);
	...
	// render the collage
	renderLayers() {
		if (this.props.layersOrder.length === 0) {
		...
		this.props.layersOrder.forEach((layer, index) => {
		...
		setTimeout(() => {
			...
			Object.assign([], this.props.layersOrder).reverse().forEach((layer) => {
...
export default connect(
	state => ({
		...
		layersOrder: state.layers.layersInOrder		// the ordered layers array (ids only)
	})
)(LayerRenderer);